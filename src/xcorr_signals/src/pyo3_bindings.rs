...
/// Accept an XCorrScaling enum member or its lowercase string form.
fn parse_scaling_any(any: &Bound<'_, PyAny>) -> PyResult<XCorrScaling> {
    if let Ok(s) = any.extract::<XCorrScaling>() {
        return Ok(s);
    }
    ...
    Err(PyTypeError::new_err(
        "scaling must be an XCorrScaling member or one of 'normalized', 'coeff', 'biased', 'unbiased', 'none'",
    ))
}
...
#[pyfunction]
#[pyo3(signature = (signals, reference, hilbert_envelope=false, n_lags=None, scaling=None, zero_pad=false))]
fn xcorr(
    py: Python<'_>,
    ...
    let default_scaling = PyString::new(py, "normalized");
    let scaling_ref: &Bound<'_, PyAny> = scaling
        .as_ref()
        .map_or(default_scaling.as_any(), |s| s.as_any());
    let scaling = parse_scaling_any(scaling_ref)?;
...
#[pyfunction]
#[pyo3(signature = (signals, reference, frame_size, hop_size, hilbert_envelope=false, n_lags=None, scaling=None, reliability_threshold=0.5))]
...
#[pyfunction]
#[pyo3(signature = (signals, reference, frame_size, hop_size, hilbert_envelope=false, n_lags=None, scaling=None))]
